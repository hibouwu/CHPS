# 项目与练习

[笔记首页](README.zh-CN.md) · [GitHub 主页](https://github.com/hibouwu)

## CHPS 课程项目

| 课程 | 项目仓库 | 内容 |
|---|---|---|
| PPN | [ppn-mnist-neural-network](https://github.com/hibouwu/ppn-mnist-neural-network) | C++ 神经网络、自动微分与训练系统 |
| CN | [cn-poisson-tools](https://github.com/hibouwu/cn-poisson-tools) | 一维 Poisson／热方程、直接法与迭代法 |
| TOP | [d2q9-lbm-karman](https://github.com/hibouwu/d2q9-lbm-karman) | MPI/OpenMP D2Q9 LBM 卡门涡街仿真与优化 |

## TD / TP 与其他课程项目

题目、解答和实验笔记按课程查找。部分课程实验尚未确认可公开访问的独立仓库，因此这里保留笔记入口。

| 课程 | 练习或项目 |
|---|---|
| [AP](M1/AP/Exercises/README.md) | 体系结构 TP |
| [APP](M1/APP/Exercises/README.md) | MPI、PThread、OpenMP TD/TP |
| [CN](M1/CN/Exercises/README.md) | 气候模拟、线性方程组与数值计算练习 |
| [POO](M1/POO/Exercises/README.md) | C++ TD/TP、粒子模拟器 |
| [GLHPC](M1/GLHPC/Exercises/README.md) | Lab 1–7、SGEMM 与推理实验 |
| [AISE](M1/AISE/Exercises/README.md) | 操作系统实验、EXPRESSO |
| [VS](M1/VS/Exercises/README.md) | ParaView 练习、储罐减压可视化 |

## 教师提供的代码与材料

- [GLHPC 课程仓库](https://github.com/M1-CHPS/glhpc)
- [TOP 实验代码](https://github.com/dssgabriel/TOP-26)
- [TOP 课程网站源码](https://github.com/dssgabriel/CHP203-TOP)
- [POO 课程仓库（GitLab）](https://gitlab.com/sdcm/chps2025)

这些是教师课程入口，与个人项目仓库分开列出。